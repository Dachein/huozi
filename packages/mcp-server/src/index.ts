...
server.tool(
  "huozi_signup",
  "Sign in or register a Huozi account. Sends a verification code to the email. No password needed.",
  {
...
server.tool(
  "huozi_verify",
  "Verify email with the code received. Returns an access_token for setup.",
  {
...
server.tool(
  "huozi_setup",
  "Create a workspace and generate an API key. Requires the access_token from huozi_verify.",
  {
  ...
  async ({ access_token, workspace_slug }) => {
    ...
    const workspace = res.data.workspace as { slug: string; url: string };
    return {
      content: [
        {
          type: "text" as const,
...
server.tool(
  ...
  async () => {
    ...
    const pages = res.data.pages as Array<{
      slug: string;
      title: string;
    ...
    if (!pages || pages.length === 0) {
      return {
        content: [{ type: "text" as const, text: "No pages yet." }],
      };
    }
    ...
    const list = pages
      .map(
        (p) =>
          `- ${p.title} (/${p.slug}) ${p.is_published ? "✓" : "[draft]"} — ${p.updated_at}`
      )
      .join("\n");
