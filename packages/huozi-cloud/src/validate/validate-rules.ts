...
export type HuoziFormat = 'deck' | 'story' | 'paper' | 'dashboard' | 'app' | 'blog'
...
export const VALIDATION_RULES: ValidationRule[] = [
  ...
  },
  {
    ...
    remedy:
      'Declare one of: `<meta name="huozi:background" content="#0b0d12">` (canvas-bleed color), a `<style>` rule on `html` / `body` / `:root` with `background: ...`, or `<body style="background: ...">`.',
    docRef: 'norms#1-format-types',
