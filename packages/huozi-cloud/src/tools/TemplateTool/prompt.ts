...
export function templatePrompt(): string {
  ...
  lines.push(
    ...
    '  • mobile H5 / miniapp workflow     → app',
    ...
    '',
    '  Canvas formats (deck / story / dashboard / app):',
    ...
    '      app:        390 ×  844  (mobile UI surface, contain fit)',
    '    deck + dashboard + app use FIT=contain (whole canvas visible,',
    '    letterbox on the leftover axis). story uses FIT=cover (canvas fills both',
    ...
    '',
    'Dashboard carries [data-tab] markers plus <meta name="huozi:tabs"',
    'content="overview=Overview, queue=Queue">. The host renders one',
    'standard tab bar outside the author iframe and switches sections via',
    'the bridge. App is single-screen today; future L2/pass-sheet concepts',
    'should be declared by platform convention rather than custom chrome.',
    '',
    ...
    '    Override default fit. story may use cover; dashboard/app always contain so UI/data is never clipped.',
    '',
    ...
    '    default to #000. dashboard / app should declare this explicitly;',
    '    paper / blog normally let the surrounding reader surface show through.',
    '',
    'Runtime isolation — Huozi renders author HTML in an iframe inside the',
    'standard canvas shell. Host chrome (page outline, dashboard tabs,',
    'fullscreen buttons) stays outside the iframe; author scripts should use',
    'data-page / data-tab declarations and window.huozi APIs, not parent DOM.',
    '',
    'Host background — inside the iframe, background on html/body/:root',
    'paints the author document. For canvas formats, `huozi:background`',
    'also paints the outer letterbox/bleed area so workspace, fullscreen,',
    '/p, and /o do not disagree at the edges.',
    '',
    'Workflow:',
    '  1. huozi_template({ format: "deck" })  ← pick from { blog, deck, story, paper, dashboard, app }',
