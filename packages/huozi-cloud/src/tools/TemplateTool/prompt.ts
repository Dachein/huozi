...
export function templatePrompt(): string {
  ...
  lines.push(
    ...
    '',
    'Two groups:',
    '  • Unpaginated (long scroll): web, mobile. No [data-page] markers.',
    '  • Paginated (one page at a time): deck, story, paper. Each unit is a',
    '    <section data-page id="sN" data-title="…"> (or <article> for paper).',
    '    huozi renders an outline + pager from these markers.',
    '',
    'Every template ships with `<meta name="huozi:format" content="X">` in',
    '<head>. Preserve it on edit; omitting / removing it makes the host',
    'treat the file as plain "web" (long scroll, no pager).',
    '',
    'Workflow:',
