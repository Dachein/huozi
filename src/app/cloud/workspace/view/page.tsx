...
import { WorkspaceShell } from "@/components/cloud/workspace-shell";
import { FileRenderer } from "@/components/cloud/file-renderer";
import { CloudLiveEvents } from "@/components/cloud/cloud-live-events";
import { LiveUpdateBanner } from "@/components/cloud/live-update-banner";
import { FileActionsMenu } from "@/components/cloud/file-actions-menu";
...
export default async function CloudFileView({ searchParams }: SearchParams) {
  ...
  if (!key) redirect("/cloud/connect");
  if (!path) redirect("/cloud/workspace");
...
function Pagination({
  ...
  const buildUrl = (newOffset: number): string => {
    ...
    return `/cloud/workspace/view?${qs.toString()}`;
  };
