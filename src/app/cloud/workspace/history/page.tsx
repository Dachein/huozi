...
function Breadcrumb({ path }: { path: string }) {
  return (
    <div className="text-xs text-muted-foreground font-mono flex items-center flex-wrap gap-x-1.5 gap-y-1">
      ...
      <Link
        href={`/cloud/workspace/view?path=${encodeURIComponent(path)}`}
        ...
        <>
          ...
          {histRes.data.has_more && histRes.data.next_before && (
            <div className="text-center">
              <Link
                href={`/cloud/workspace/history?path=${encodeURIComponent(path)}&before=${encodeURIComponent(histRes.data.next_before)}&limit=${limit}`}
