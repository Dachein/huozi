 ...
 *
 * Two consumers:
 *   1. `validateHuoziHtml(html)` in ./validate.ts — runs detection,
 *      emits issues that reference these `code`s.
 *   2. `huozi_validate_rules` MCP tool — agents can query this catalog
 *      to learn the full ruleset BEFORE writing, instead of waiting
 *      for the validator to flag mistakes after the fact.
 *
 * Severity contract:
 *   - **error**: write WILL produce broken render (paginated with no
 *     pages, unknown format value, duplicate page ids). WriteTool
 *     gates on these — error means the write is refused.
 *   - **warning**: write succeeds; the result probably isn't what the
 *     author intended (CDN script will be stripped, format meta
 *     disagrees with class). Returned to the agent via tool result so
 *     the next pass can fix.
 *   - **hint**: best-practice nudge. Pure FYI; agents can ignore.
 *
 * Each rule has a `docRef` slug pointing into the live HTML spec doc
 * (`dev/2026-05-10-share-viewer-norms.html`). When the spec moves, the
 * slugs come along; the rule code is the stable foreign key.
 */
...
export interface ValidationRule {
  /** Stable kebab-case identifier. Same value used in `ValidationIssue.code`. */
  code: string;
  level: ValidationLevel;
  /** Short one-line label. Used for tool-list summaries. */
  title: string;
  /** Plain-language explanation of what the rule guards against. */
  why: string;
  /** Concrete next step the agent should take. */
  remedy: string;
  /** Optional anchor into the live spec doc. */
  docRef?: string;
  /** Optional format filter; rule is inert outside this set. */
  appliesTo?: HuoziFormat[];
}

/** The full ordered list of rules. validateHuoziHtml emits issues in
 *  this order so the UI banner reads top-to-bottom by severity-ish
 *  priority (error first, then warning, then hint). */
export const VALIDATION_RULES: ValidationRule[] = [
  // ── Format declaration ──────────────────────────────────────────────
  {
    ...
    remedy: 'Set `<meta name="huozi:format" content="blog">`. blog is the responsive long-form format that covers both phone and desktop.',
  ...
  },
  {
    ...
    remedy: "Align the two: pick one format and update both the meta and the `<div class=\"huozi-X\">` wrapper.",
  ...
  // ── Paginated structure ────────────────────────────────────────────
  {
    ...
    remedy: 'Wrap each slide / page in `<section data-page id="sN" data-title="Page title">`.',
  ...
  },
  {
    ...
    remedy: "Give each data-page a unique id, or leave id off entirely (the platform will auto-assign).",
    docRef: "norms#2-page-marker",
    appliesTo: ["deck", "story", "paper"],
  },
  {
  ...
  // ── Sandbox / strip rules ──────────────────────────────────────────
  {
    ...
    remedy: 'For known libraries (mermaid, echarts, etc.) declare `<meta name="huozi:bundle" content="mermaid,echarts">` — the platform injects them in a sandbox-aware way.',
  ...
  },
  {
    ...
    remedy: "Replace with native HTML/CSS, screenshot+link, or platform-supported bundles.",
    docRef: "toolbox-spec#3-2-author-constraints",
  },
  {
    ...
    remedy: "Check the supported list and fix the typo, or drop the key and inline the library yourself if it's small.",
  ...
  // ── Geometry / scaling hygiene ─────────────────────────────────────
  {
    ...
    remedy: "Use cqw / cqh instead. The platform sets `container-type: size` on the canvas root; cqw / cqh resolve to the canvas's pixel dimensions, identical across all three render modes.",
  ...
  },
  {
    ...
    remedy: 'Add `<meta property="og:image" content="https://…">` or leave blank to use the huozi default.',
    docRef: "norms#5-share-card",
  },
];

/** Look up rule metadata by code. Returns null for unknown codes. */
export function getRule(code: string): ValidationRule | null {
  return VALIDATION_RULES.find((r) => r.code === code) ?? null;
}

/** Full catalog accessor for tools / docs. Defensive copy so callers
 *  can't mutate. */
